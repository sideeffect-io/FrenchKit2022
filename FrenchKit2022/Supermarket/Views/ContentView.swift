...
struct ContentView: View {
  ...
  var body: some View {
    NavigationView {
      VStack {
        ...
        // the shopping cart at the checkout
        Group {
  ...
  var cashier: some View {
    HStack {
      Button {
        stateMachine.send(.givingMyCreditCard(CreditCard(number: "1343 3243 2343 3421", pin: 1234)))
      } label: {
  ...
  var aisle: some View {
    VStack {
      HStack(alignment: .top) {
        ...
        Button {
          self.stateMachine.send(.walkingToCashier)
        } label: {
